...
import { Card, Button, Field, TextField } from "@eyf/ui";
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-10 py-8 lg:py-12 max-w-2xl">
      ...
      <Card className="mt-8 space-y-4">
        ...
        <TextField label="Job title" value={form.jobTitle} onChange={(v) => setForm({ ...form, jobTitle: v })} />
        <TextField label="Years experience" type="number" value={String(form.yearsExp)} onChange={(v) => setForm({ ...form, yearsExp: Number(v) })} />
        <TextField label="Expertise (comma-separated)" value={form.expertise} onChange={(v) => setForm({ ...form, expertise: v })} />
        <TextField label="Hourly rate (₹)" type="number" value={String(form.hourlyRateInr)} onChange={(v) => setForm({ ...form, hourlyRateInr: Number(v) })} />
        <Field label={<>Bio <span className="text-text-4 normal-case tracking-normal">(min 50 chars)</span></>}>
          ...
          <div className={`text-xs mt-1 ${form.bio.trim().length < 50 ? "text-text-4" : "text-easy"}`} aria-live="polite">{form.bio.trim().length}/50</div>
        </Field>
