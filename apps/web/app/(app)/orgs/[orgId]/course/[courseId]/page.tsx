          ...
          <div className="min-w-0 space-y-1.5">
            {course.lessons.map((l) => (
              <button key={l.id} onClick={() => setActiveId(l.id)}
                className={`w-full text-left rounded-lg px-3 py-2 text-sm flex items-center gap-2 transition-colors ${activeId === l.id ? "bg-surface border border-accent/50" : "hover:bg-surface-3"}`}>
                <span className={`shrink-0 ${l.completed ? "text-easy" : "text-text-4"}`}>{l.completed ? "✓" : "○"}</span>
