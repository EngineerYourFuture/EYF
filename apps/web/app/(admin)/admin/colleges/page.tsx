    ...
    <div className="px-4 sm:px-6 lg:px-10 py-8 max-w-6xl mx-auto">
      ...
      <div className="grid lg:grid-cols-[320px_1fr] gap-6">
        <Card>
          ...
          <CollegeList colleges={colleges} isLoading={isLoading} selected={selected} onSelect={setSelected} />
        </Card>

        <div>
          <BatchDetail selected={selected} batch={batch} />
        </div>
      </div>
    </div>
  );
...
function CollegeList({
  ...
  if ((colleges ?? []).length === 0) {
    return <p className="text-text-4 text-sm">No colleges yet — students add theirs in Settings.</p>;
  }
  return (
...
function BatchDetail({ selected, batch }: Readonly<{ selected: string | null; batch: Batch | undefined }>) {
  if (!selected) {
    return (
  ...
  if (!batch) return <Card><SkeletonRows rows={4} /></Card>;
  return (
    <Card>
