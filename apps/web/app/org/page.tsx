...
export default function OrgPortal() {
  ...
  useEffect(() => { try { setCode(localStorage.getItem(CODE_KEY)); } catch { /* */ } setReady(true); }, []);

  const api = useCallback(async (path: string, opts: RequestInit = {}) => {
    const res = await fetch(`${API}/org${path}`, {
      ...opts,
      headers: { "content-type": "application/json", "x-org-code": code ?? "", ...(opts.headers ?? {}) },
    });
  ...
  function logout() { try { localStorage.removeItem(CODE_KEY); } catch { /* */ } setCode(null); setOrg(null); }
  async function login(e: React.FormEvent) {
    ...
    try {
      ...
      const j = await res.json();
      if (!j.success) { setErr("That code doesn't match any organisation."); return; }
      try { localStorage.setItem(CODE_KEY, codeInput.trim()); } catch { /* */ }
