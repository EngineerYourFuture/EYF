...
const MODULES = [
  { num: '01', title: 'Data Structures & Algorithms', desc: 'Arrays to graphs, DP to divide-and-conquer.', color: 'from-red-500/10' },
  { num: '02', title: 'Core CS Fundamentals', desc: 'OS scheduling, ACID transactions, CAP theorem.', color: 'from-blue-500/10' },
  { num: '03', title: 'Placement Prep', desc: 'Behaviorals, system design, and timed coding rounds.', color: 'from-purple-500/10' },
  { num: '04', title: 'Resume Engineering', desc: 'ATS-optimised bullet points with real recruiter feedback.', color: 'from-green-500/10' },
  { num: '05', title: 'Modern Tech Stack', desc: 'React, Node, cloud platforms and DevOps in production.', color: 'from-yellow-500/10' },
  { num: '06', title: 'Elite Mentorship', desc: 'Structured coaching from engineers at FAANG and unicorns.', color: 'from-pink-500/10' },
];
...
/* Reusable section wrapper — fills full width with proportional padding */
const S = 'w-full px-[5vw] 2xl:px-[6vw]';
...
export function LandingPage() {
  return (
    <div className="dark min-h-screen bg-[#0e0e0e] text-[#e2e2e2] overflow-x-hidden">
      ...
      {/* ── Navbar ── */}
      <header className="fixed top-4 left-0 right-0 px-[3vw] z-50">
      ...
      <main>
        ...
        <section className="relative min-h-screen flex items-center justify-center pt-24 pb-20 overflow-hidden">
          ...
          <div className="relative z-10 w-full px-[5vw] text-center">
            ...
            <h1 className="text-[clamp(3rem,9vw,8rem)] font-black tracking-[-0.04em] leading-[0.88] mb-8 text-white">
              Engineer Your<br /><span className="text-[#E82127]">Future.</span>
            </h1>
        ...
        {/* ── Features ── */}
        <section id="features" className={`${S} py-28`}>
          <div className="mb-16">
          ...
          {/* 1→2→3→6 columns so wide monitors see one clean row */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6 gap-5">
            {FEATURES.map((f) => (
              <div
                key={f.title}
                className="group relative bg-[#161616] border border-white/[0.06] rounded-2xl p-7 hover:border-white/10 hover:bg-[#1a1a1a] transition-all duration-300 overflow-hidden flex flex-col"
              >
                ...
                <div className="relative flex flex-col flex-1">
                  <div className="flex items-start justify-between mb-6">
                    <div className="w-10 h-10 rounded-xl bg-[#E82127]/10 flex items-center justify-center text-[#E82127] group-hover:bg-[#E82127] group-hover:text-white transition-all duration-300">
                      <Icon name={f.icon} size={18} />
                    </div>
                    <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-600 bg-white/5 px-2.5 py-1 rounded-full">
                      {f.tag}
                    </span>
                  </div>
        ...
        {/* ── How It Works ── */}
        <section id="how" className="py-28 relative overflow-hidden w-full">
          <div className="absolute inset-0 bg-[#E82127]/[0.03] pointer-events-none" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto relative">
              ...
              {STEPS.map((s) => (
                <div key={s.n} className="text-center">
                  <div className="w-16 h-16 rounded-full bg-[#E82127]/10 border border-[#E82127]/20 flex items-center justify-center mx-auto mb-6">
        ...
        {/* ── Curriculum ── */}
        <section id="modules" className={`${S} py-28`}>
          ...
          {/* 1→2→3→6 columns same as features */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6 gap-5">
            {MODULES.map((m) => (
              <div
                key={m.num}
                className={`relative bg-gradient-to-b ${m.color} to-transparent bg-[#161616] border border-white/[0.06] rounded-2xl p-7 hover:border-white/10 transition-all duration-300 overflow-hidden`}
              >
                <div className="absolute top-0 right-0 text-[100px] font-black text-white/[0.03] leading-none select-none pr-2 -mt-2">{m.num}</div>
                <div className="relative">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-[#E82127] bg-[#E82127]/10 px-3 py-1 rounded-full inline-block mb-4">
                    Module {m.num}
                  </span>
        ...
        {/* ── CTA ── */}
        <section className={`${S} py-16 mb-16`}>
