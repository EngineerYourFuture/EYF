...
const OAUTH_ERROR_MESSAGES: Record<string, string> = {
  ...
  oauth_no_email: 'No email returned from Google. Please use email/password login.',
          ...
          <div className="bg-surface-container rounded-xl p-8 md:p-12 shadow-[40px_0_60px_-15px_rgba(0,0,0,0.3)]">
            ...
            {tab === 'login' ? (
              <form onSubmit={onLogin} className="space-y-8">
                ...
                {require2FA && (
                  <div className="space-y-3">
                    <label htmlFor="login-totp" className="font-['Inter'] uppercase tracking-widest text-[10px] font-bold text-on-surface-variant block px-4">
                      Authenticator Code
                    </label>
                    <div className="relative">
                      <input
                        ...
                        className="w-full bg-surface-container-low border-none rounded-xl px-6 py-4 text-on-surface placeholder:text-zinc-600 focus:outline-none focus:ring-0 transition-all tracking-[0.4em] text-center font-black text-xl"
                        placeholder="000000"
                        required
                        autoFocus
                      />
                    </div>
                    <p className="text-zinc-600 text-xs px-4">Enter the 6-digit code from your authenticator app.</p>
                  </div>
                )}
