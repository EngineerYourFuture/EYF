...
export function LevelUpModal({ level, onClose }: Props) {
  ...
  return (
    ...
    >
      <div role="none" className="absolute inset-0 bg-black/80 backdrop-blur-md" onClick={handleClose} onKeyDown={(e) => { if (e.key === 'Escape') handleClose(); }} />
      ...
      >
        ...
        <div className="absolute -top-4 left-1/2 -translate-x-1/2 flex gap-1">
          {(['✨','⭐','✨'] as const).map((s, i) => (
            <span key={`star-${s}-${i}`} className="text-xl animate-bounce" style={{ animationDelay: `${i * 150}ms` }}>{s}</span>
          ))}
        </div>
